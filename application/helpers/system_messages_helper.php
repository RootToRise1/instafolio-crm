...
function _maybe_system_setup_warnings()
{
    ...
    // Check php version
    add_action('before_start_render_dashboard_content', '_check_minimum_php_version');
    ...
    add_action('before_start_render_dashboard_content', '_maybe_cron_job_is_not_working_properly');
}
 ...
 * @return null
 */
function _check_minimum_php_version()
{
    if (phpversion() < '5.6' && is_admin()) { ?>
        <div class="col-md-12">
            <div class="alert alert-warning">
                <h4><strong>PHP Version Notice</strong></h4>
                <p>
                    <strong>Starting from version 2.1.0, the minimum required php version is 5.6</strong>. You should consider upgrading your php version, after you upgrade the php version this message will disappear.
                </p>
            </div>
        </div>
    <?php }
    ...
    if (get_option('cron_has_run_from_cli') == 0) {
        if (is_admin()) {
            ...
            if ($using_cron_features > 0 && get_option('hide_cron_is_required_message') == 0) {
                ...
                echo '<br />Please follow the cron <a href="https://help.instafolio.com/setup-cron-job/" target="_blank">setup guide</a> in order all features to work well.';
                echo '<br /><br /><br />';
